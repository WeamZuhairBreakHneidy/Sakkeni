...
// import '../../payment/controllers/payment_service_provider_controller.dart';
...
class UserQuotesView extends GetView<UserQuotesController> {
        ...
        children: [
          ...
          Padding(
            ...
            child: SingleChildScrollView(
              scrollDirection: Axis.horizontal,
              child: Obx(() {
                final pendingAcceptanceCount =
                    controller.quotes
                        .where(
                          (quote) => quote.status == "Pending User Acceptance",
                        )
                        .length;
                final awaitingPaymentCount =
                    controller.quotes
                        .where(
                          (quote) =>
                              quote.serviceActivity?.status ==
                ...
                final inProgressCount =
                    controller.quotes
                        .where(
                          (quote) =>
                              quote.serviceActivity?.status == "In Progress",
                        )
                        .length;
                final completedCount =
                    controller.quotes
                        .where(
                          (quote) =>
                              quote.serviceActivity?.status == "Completed",
                        )
                        .length;
                final ratedCount =
                    controller.quotes
                        .where(
                          (quote) => quote.serviceActivity?.status == "Rated",
                        )
                        .length;
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // Cost (show only serviceActivity.cost if available, to avoid duplication)
            if (quote.serviceActivity != null &&
                quote.serviceActivity!.cost != null)
              Container(
                padding: EdgeInsets.symmetric(horizontal: 10.w, vertical: 4.h),
                decoration: BoxDecoration(
                  color: AppColors.primary.withOpacity(0.1),
      ...
      case "Pending User Acceptance":
        addButton("Confirm", Colors.green, () {
          _showConfirmation(
            "Confirm",
            quote,
            () => controller.confirmQuote(quote.id),
          );
        });
        buttons.add(SizedBox(width: 8.w));
        addButton("Decline", Colors.red, () {
          _showConfirmation(
            "Decline",
            quote,
            () => controller.declineQuote(quote.id),
          );
        });
        break;
    }
        ...
        _showConfirmation("Pay Now", quote, () {
          if (quote.serviceActivity?.id != null) {
            paymentController.makePayment(
              serviceId: quote.serviceActivity!.id.toString(),
            );
          } else {
            Get.snackbar("Error", "Service ID not available for payment.");
          }
    ...
    if (quote.serviceActivity?.status == "In Progress") {
      ...
      addButton("Mark as Complete", Colors.blue, () {
        _showConfirmation(
          "Mark as Complete",
          quote,
          () => controller.markAsComplete(quote.serviceActivity!.id),
        );
      });
    }
  ...
  void _showUpdateRequestDialog(UserQuote quote) {
    final TextEditingController jobDescriptionController =
        TextEditingController(text: quote.jobDescription);
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Icon(Icons.edit, color: AppColors.primary, size: 24.r),
                  SizedBox(width: 8.w),
                  Flexible(
                    child: Text(
                      "Update Request",
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Icon(Icons.star, color: AppColors.primary, size: 24.r),
                  SizedBox(width: 8.w),
                  Flexible(
                    child: Text(
                      "Rate Service",
